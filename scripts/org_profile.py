"""
OrgNetworkProfile — a realistic segmented-network traffic profile.

Plugs into auto_attack.py when TRAFFIC_PROFILE=org (or college for legacy compat).
generate_window() returns rows in the same schema as the generators in scripts/,
with one extra trailing column — target_zone — that NetworkMap.jsx uses to
highlight the correct zone.

Row schema (see OrgNetworkProfile.TRAFFIC_HEADER):
...
Behavior by time of day (datetime.now().hour):
  Peak      (08:00–22:00): high volume, 15% attack chance, mostly endpoint IPs
...
# Zones an external attacker is most likely to target (servers/management over endpoints).
...
class OrgNetworkProfile:
    """Generates one traffic window shaped like a real segmented org network."""
        ...
        if label == "port_scan":
            ...
            for i in range(25):
                rows.append([(start + timedelta(seconds=i % 5)).isoformat(),
                             ...
                             False, label,
                             round(random.uniform(200.0, 400.0), 2),
                             round(random.uniform(1.0, 3.0), 3),
            ...
            for i in range(40):
                rows.append([(start + timedelta(milliseconds=i * 5)).isoformat(),
                             ...
                             random.choice([True, True, False]), label,
                             round(random.uniform(500.0, 1500.0), 2),
            ...
            for i in range(15):
                rows.append([(start + timedelta(milliseconds=i * 250)).isoformat(),
                             ...
                             random.choice([True, False, False]), label,
                             round(random.uniform(350.0, 900.0), 2),
                             round(random.uniform(2.0, 8.0), 3),
            ...
            for i in range(20):
                rows.append([(start + timedelta(milliseconds=i * 25)).isoformat(),
                             ...
                             random.choice([True, True, True, False]), label,
                             round(random.uniform(800.0, 2000.0), 2),
                             round(random.uniform(5.0, 30.0), 3),
...
# Legacy alias — keeps any code importing CollegeNetworkProfile working.
CollegeNetworkProfile = OrgNetworkProfile
...
if __name__ == "__main__":
    ...
    with open("data/raw/org_traffic.csv", "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(OrgNetworkProfile.TRAFFIC_HEADER)
        w.writerows(window)
    print(f"Org window generated — peak={profile.is_peak()}, rows={len(window)}")
