...
def generate_sql_injection(n_requests: int = 40, target_ip: str = "192.168.1.10",
    ...
    for i in range(n_requests):
        rows.append([
            ...
            "sql_injection",
            round(random.uniform(350.0, 900.0), 2),      # bytes_per_packet (SQL payloads)
            round(random.uniform(2.0, 8.0), 3),          # connection_duration
            round(random.uniform(2.0, 3.5), 2),          # payload_entropy (structured SQL is low)
        ])
    return rows
